...
using Object = core::HashMap<core::String, Value>;
struct Error {
    enum ErrorKind {
...
#define GETTER_VAL(T, kind) core::Option<T> get_##T() const { if (m_kind == kind) { return core::Option<T>::Some( std::get<T>(m_data)); } else { return core::Option<T>::None(); } }
#define GETTER_REF(T, kind, name) core::Option<const T*> get_##name() const { if (m_kind == kind) { return core::Option<const T*>::Some(&std::get<T>(m_data)); } else { return core::Option<const T*>::None(); } }
#define GETTER_MUT(T, kind, name) core::Option<T*> get_##name##_mut() { if (m_kind == kind) { return core::Option<T*>::Some(&std::get<T>(m_data)); } else { return core::Option<T*>::None(); } }

    GETTER_VAL(bool,         JSON_BOOL)
    GETTER_VAL(i64,          JSON_I64)
    GETTER_VAL(u64,          JSON_U64)
    GETTER_VAL(f64,          JSON_F64)
...
#define OBJECT(obj) obj.get_object().unwrap()
#define OBJECT_MUT(obj) obj.get_object_mut().unwrap()
#define OBJECT_GET(obj, key) obj.get_object().unwrap()->get(key)
#define OBJECT_GET_MUT(obj, key) obj.get_object_mut().unwrap()->get_mut(key)
#define OBJECT_INSERT(obj, key, val) obj.get_object_mut().unwrap()->insert(key, val)
#define ARRAY(obj) obj.get_array().unwrap()
#define ARRAY_MUT(obj) obj.get_array_mut().unwrap()
#define ARRAY_GET(obj, index) obj.get_array().unwrap()->get(index)
#define ARRAY_GET_MUT(obj, index) obj.get_array_mut().unwrap()->get_mut(index)
#define ARRAY_PUSH(obj, val) obj.get_array_mut().unwrap()->push(val)
